...
#include "ui_ziothomecontroldialog.h"
...
std::list<std::string> ZIoTHomeControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZIoTHomeControlDialog::listMints;

ZIoTHomeControlDialog::ZIoTHomeControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZIoTHomeControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZIoTHomeControlDialog::~ZIoTHomeControlDialog()
{
    delete ui;
}

void ZIoTHomeControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZIoTHomeControlDialog::updateList()
{
...
void ZIoTHomeControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZIoTHomeControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZIoTHome_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZIoTHomeControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZIoTHomeControlDialog::GetSelectedMints()
{
...
void ZIoTHomeControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
