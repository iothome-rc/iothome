...
#include "ziothomecontroldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zIOTH ought to be enough for anybody." - Bill Gates, 2017
    ui->zIOTHpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzIOTHSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzIOTH->setEnabled(false);
        ui->pushButtonMintzIOTH->setToolTip(tr("zIOTH is currently disabled due to maintenance."));

        ui->pushButtonSpendzIOTH->setEnabled(false);
        ui->pushButtonSpendzIOTH->setToolTip(tr("zIOTH is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zIOTHpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzIOTH_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zIOTH is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zIOTH...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zIOTH in ") +
...
void PrivacyDialog::on_pushButtonSpendzIOTH_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zIOTH is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zIOTH
        sendzIOTH();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zIOTH
    sendzIOTH();
}

void PrivacyDialog::on_pushButtonZIoTHomeControl_clicked()
{
    ZIoTHomeControlDialog* zIoTHomeControl = new ZIoTHomeControlDialog(this);
    zIoTHomeControl->setModel(walletModel);
    zIoTHomeControl->exec();
}

void PrivacyDialog::setZIoTHomeControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzIoTHomeSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzIOTH()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid IoTHome Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zIOTHpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zIOTH is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " IOTH </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zIOTH</b>";
    ...
    // use mints from zIoTHome selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZIoTHomeControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZIoTHomeControlDialog::GetSelectedMints();
    }
    ...
    // Clear ziothome selector in case it was used
    ZIoTHomeControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zIoTHome Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " IoTHome, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zIoTHome Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zIOTH </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zIOTH "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zIOTH "));
    ui->labelzIOTHAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzIOTHSyncStatus->setVisible(fShow);
}
